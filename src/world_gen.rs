...
impl PreWorld{
    pub fn gen_world_from_builder(builder:WorldBuilder)->Result<PreWorld,LibError>   {
        ...
        let ( mut world, elevation_vec) = noise_function.generate_tiles_and_elevation(size,noise_to_tile,elevation_factor);
...
impl Generator for PreWorld{
    fn gen(&mut self)->World {
        ...
        (tiles,(self.size,self.size),self.environmental_conditions.clone(),self.max_score,score_table)
