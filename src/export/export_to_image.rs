...
use robotics_lib::world::tile::{Content, Tile, TileType};
    ...
    pub fn export_to_image(filename: &str, map: &Vec<Vec<Tile>>)->Result<(), String> {
        let (width, height) = (map.len(), map.len());
        ...
        for x in 0..width {
            for y in 0..height {
                let tile_color = color_for_tile(&map[x][y]);
        ...
        match image.save(filename){
            Ok(_) => {Ok(())}
            Err(_) => {Err("Couldn't save image".into())}
        }
    }
...
fn color_for_tiletype(tile: &TileType) -> Rgb<u8> {
    ...
    match tile {
        ...
        TileType::Teleport(_) => Rgb([0,100, 200]),
        _ => Rgb([0,0,0]),
    }
}
